...
class Serve extends Component {
  static propTypes = {
    ...
    getMediaParams: PropTypes.shape(),
  ...
  componentDidMount() {
    ...
    const io = new IntersectionObserver(
      async entry => {
        if (
          this.props.mediaUuid &&
          (this.props.media.type.includes("image") ||
            (this.props.media.extension.includes("pdf") &&
            ...
            this.promise = serve(
              {
                ...
                mediaUuid: this.props.mediaUuid,
                getMedia: this.props.getMedia
              },
              { ...this.props.getMediaParams }
            );
  ...
  async componentWillReceiveProps(nextProps) {
    if (nextProps.mediaUuid && nextProps.mediaUuid !== this.props.mediaUuid) {
      ...
      if (
        nextProps.media.type.includes("image") ||
        (nextProps.media.extension.includes("pdf") && nextProps.showContent)
      ) {
        this.promise = serve(nextProps.mediaUuid, nextProps.getMedia, {
          ...nextProps.getMediaParams
        });
        const url = await this.promise.call();
        if (url && !this.unmounted) {
          this.setState({ url });
      ...
      if (this.props.background) {
        return (
          <div
            ...
            className={`${this.props.classNames.image}${
              this.state.url ? " is-loaded" : ""
            }`}
            style={{
              backgroundImage: `url('${
                this.state.url ? this.state.url : loadingPic
              }')`
            }}
          />
        );
      ...
      return (
        ...
        >
          <object
            ...
            width="100%"
            height="100%"
            data={this.state.url ? this.state.url : loadingPic}
          />
        </div>
      );
    }
